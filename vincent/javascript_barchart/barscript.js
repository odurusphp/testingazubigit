//create an array to store month and sales
const monthlySales = [
...
function chartBar() {
    ...
    var percentage = monthlySales.map((num)=>{
        return ((num.amount/sum)*100).toFixed(1)
    });
    
    //List the sales per month
    var salesPerMonthList = monthlySales.map((amt)=>{
        return amt.amount   });
    
    //lists the months in the year
    var monthlList = monthlySales.map((months)=>{
        return months.month   });

    //Initializing HTML document elements
    const DIV_ITEMS = document.querySelector(".items");
    const DIV_ITEMS_BOTTOM = document.querySelector(".items-bottom");

    //Looping statement to loop through the months and append data in the HTML document
